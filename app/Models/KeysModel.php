...
class KeysModel extends Model
{
    ...
    protected $allowedFields = ['game', 'user_key', 'user_pass', 'duration', 'expired_date', 'max_devices', 'devices', 'status', 'registrator'];
    ...
    public function API_getKeys()
    {
        ...
        $builder->select('CONCAT(keys_code.id_keys) as id, game, user_key, user_pass, duration, CONCAT(keys_code.expired_date) as expired, max_devices, devices, status, registrator');

        return DataTable::of($builder)
            ->setSearchableColumns(['id_keys', 'game', 'user_key', 'user_pass', 'duration', 'expired_date', 'max_devices', 'devices', 'registrator'])
